...
def train_challenge_model(data_folder, model_folder, verbose):
    ...
    pat = 15 #5 # the number of epochs with no improvement after which the training will stop
    early_stopping =  EarlyStopping(monitor='val_loss',patience=pat, verbose=1, restore_best_weights=True)

    n_folds = 3#10
    epochs = 120
    batch_size = 32#1#2#4#8#16#32#64
    ...
    for train_index, cv_index in kf.split(X_all,y1_all,groups=group):
        ...
        if selector == 1:
            ...
            outcome_filename = os.path.join(model_folder, '10-fold CV', 'Fold_' + str(k) + '_outcome_KFold.h5')
        else:
            ...
            outcome_filename = os.path.join(model_folder, '10-fold CV', 'Fold_' + str(k) + '_outcome_StratifiedKFold.h5')
        outcome_model_checkpoint = ModelCheckpoint(filepath = outcome_filename, monitor='val_loss', verbose=1, save_best_only=True, mode='min')
        ...
        if np.max(outcome_result.history['val_loss'])<np.min(outcome_result.history['loss']):
            # k = k-1
            outcome_res.append(0)
        else:
    ...
    if selector == 1:
        shutil.copyfile(os.path.join(model_folder, '10-fold CV', 'Fold_' + str(np.argmax(outcome_res)+1) + '_outcome_KFold.h5'), new_file_name)
        ...
        shutil.copyfile(os.path.join(model_folder, '10-fold CV', 'Fold_' + str(np.argmax(outcome_res)+1) + '_outcome_StratifiedKFold.h5'), new_file_name)
        #shutil.copyfile(os.path.join(model_folder, '10-fold CV', 'Fold_1' + '_outcome_StratifiedKFold.h5'), new_file_name)
        ...
        #         pass
        # shutil.copyfile(fold_best_model[Ind[np.argmin(Temp)]], new_file_name)
        # #shutil.copyfile(fold_best_model[0], new_file_name)
    print('Select final model from Fold_' + str(np.argmax(outcome_res)+1))
    #print('Select final model from Fold_1')
    ...
    # set early stopping criteria
    pat = 5 #5 # the number of epochs with no improvement after which the training will stop
    early_stopping =  EarlyStopping(monitor='val_loss',patience=pat, verbose=1, restore_best_weights=True)
    ...
    for train_index, cv_index in kf.split(X_all,y2_all,groups=group):
        ...
        if selector == 1:
            ...
            cpc_filename = os.path.join(model_folder, '10-fold CV', 'Fold_' + str(k) + '_cpc_KFold.h5')
        else:
            # cpc_filename = os.path.join(model_folder, '10-fold CV', 'Fold_' + str(k) + '_{epoch:03d}_{val_mse:.4f}' + '_cpc_StratifiedKFold.h5')
            cpc_filename = os.path.join(model_folder, '10-fold CV', 'Fold_' + str(k) + '_cpc_StratifiedKFold.h5')
        cpc_model_checkpoint = ModelCheckpoint(filepath = cpc_filename, monitor='val_mse', verbose=1, save_best_only=True, mode='min')
    ...
    if selector == 1:
        shutil.copyfile(os.path.join(model_folder, '10-fold CV', 'Fold_' + str(np.argmin(cpc_res)+1) + '_cpc_KFold.h5'), new_file_name)
        # fold_best_model = list()
        # fold_best_metric = list()
        # for i in range(n_folds):
        #     temp = list()
        ...
        shutil.copyfile(os.path.join(model_folder, '10-fold CV', 'Fold_' + str(np.argmin(cpc_res)+1) + '_cpc_StratifiedKFold.h5'), new_file_name)
        # fold_best_model = list()
        # fold_best_metric = list()
        # for i in range(n_folds):
        #     temp = list()
        ...
        # shutil.copyfile(fold_best_model[np.argmin(fold_best_metric)], new_file_name)
    print('Select final model from Fold_' + str(np.argmin(cpc_res)+1))
...
def load_challenge_models(model_folder, verbose):
    ...
    return [load_model(outcome_filename, custom_objects={'macro_double_soft_f1_loss': macro_double_soft_f1_loss}), load_model(cpc_filename)] #, 'macro_double_soft_f1': macro_double_soft_f1 #, 'macro_double_soft_tpr': macro_double_soft_tpr
...
def macro_double_soft_f1_loss(y, y_hat):
    ...
    # soft_f1_class1 = 1.04*tp / (1.04*tp + 0.04*fn + fp + 1e-16)#1.25*tp / (1.25*tp + 0.25*fn + fp + 1e-16)
...
def generate_cnn(k, X_all, feature_all):
    ...
    model = Conv2D(16, (3,3), padding = 'same', activation='relu', kernel_initializer='he_normal')(EEG_input)
    #model = tf.keras.layers.LeakyReLU(alpha=0.01)(model)
    model = BatchNormalization(axis=-1)(model)
    model = MaxPool2D(pool_size=(2,2), padding = 'same')(model)
    ...
    sgd =tf.keras.optimizers.legacy.SGD(lr=0.0005) #, decay=1e-6, momentum=0.9 #lr=0.000001
    ...
    if k ==2:
        new_model.compile(optimizer = sgd,#RMSprop,#tf.keras.optimizers.Adam(0.001),
              loss=macro_double_soft_f1_loss,#tf.keras.losses.BinaryFocalCrossentropy(alpha=0.25, gamma=2.0),#f1_loss,#BinaryFocalLoss(gamma=2, pos_weight=0.15),#focal_loss(), #tfr.keras.losses.PairwiseHingeLoss(),#custom_loss(), #'categorical_crossentropy',
              metrics=[tf.keras.metrics.Recall()]) #,tf.keras.metrics.SensitivityAtSpecificity(specificity=0.95) #,tf.keras.metrics.AUC() # tf.keras.metrics.binary_focal_crossentropy(alpha=0.25, gamma=2.0) # macro_double_soft_tpr,  # tf.keras.metrics.SensitivityAtSpecificity(specificity=0.95),  #['accuracy'])
    else:
...
def fit_and_eval(X_train, X_val,fea_train, fea_val,y_train, y_val,model,epochs,batch_size,early_stopping,model_checkpoint):
    ...
    cnn_model = None
    cnn_model = model
    results = cnn_model.fit([X_train, fea_train], y_train, epochs=epochs,batch_size=batch_size,callbacks = [early_stopping,model_checkpoint], verbose=1, validation_data = ([X_val, fea_val],y_val)) # 
...
def plot_figures(A, n_folds,model_history, model_folder):
    ...
    for i in range(n_folds):
        ...
        if A == 'outcome':
            ...
            # plt.plot(model_history[i].history['sensitivity_at_specificity'], label = 'Training sensitivity_at_specificity')
            # plt.plot(model_history[i].history['val_sensitivity_at_specificity'], label = 'Validation sensitivity_at_specificity', linestyle = 'dashdot')
