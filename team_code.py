...
def train_challenge_model(data_folder, model_folder, verbose):
    ...
    pat = 10 # the number of epochs with no improvement after which the training will stop
    early_stopping =  EarlyStopping(monitor='val_loss',patience=pat, verbose=1, restore_best_weights=True)

    res = list()
    for T in range(1): # run 5 times to save memory
        ...
        # train models   
        outcome_results, outcome_val_score, outcome_indices = fit_and_eval(X_train, y1_train, z_all, outcome_model, epochs, batch_size, early_stopping, oucome_model_checkpoint)
        res.append(outcome_val_score[1])
        cpc_results, cpc_val_score, cpc_indices = fit_and_eval(X_train, y2_train, z_all, cpc_model, epochs, batch_size, early_stopping, cpc_model_checkpoint)
...
def generate_cnn(k, X_all):
    ...
    model = Sequential()
    model.add(Conv2D(16, (3,3), padding = 'same', activation='relu', input_shape=(np.shape(X_all)[1],np.shape(X_all)[2],np.shape(X_all)[3])))
    ...
    if k ==2:
        model.compile(optimizer = tf.keras.optimizers.legacy.SGD(learning_rate=0.0005),#tf.keras.optimizers.Adam(0.0005),
...
def fit_and_eval(X,y,z,model,epochs,batch_size,early_stopping,model_checkpoint):
...
def plot_figures(A, n_folds,model_history, model_folder, T):
